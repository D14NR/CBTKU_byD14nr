 ...
 * Menyimpan/memperbarui jawaban gabungan semua mapel
 */
router.post('/save-gabungan-jawaban', async (req, res) => {
  ...
  try {
    ...
    // Hitung total soal dari jawaban_all (dipisah oleh |)
    const total_soal = jawaban_all.split('|').length;

    // Cek apakah sudah ada data untuk peserta-agenda ini
 ...
 * Mendapatkan semua jawaban gabungan dalam satu agenda (untuk admin)
 */
